...
- **5.6 Design verification (bắt buộc trước Done)**: màn hình mockup-driven phải chạy `node apps/web/scripts/verify-design.mjs` (audit computed styles mockup vs app, desktop + mobile) → **DIFF phải = 0** hoặc ghi rõ lý do chấp nhận vào `docs/13-mockups/design-verification/design-verification-login.md` (file báo cáo tự sinh, là chuẩn đối chiếu cho thiết kế sau). Không báo Done khi chưa chạy audit.
